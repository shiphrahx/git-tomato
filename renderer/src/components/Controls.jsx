...
export function Controls({ status, type, onStart, onPause, onStop, onReset, onStartShortBreak, onStartLongBreak, onConfig }) {
  ...
  return (
    <div className="controls">
      ...
      <div className="controls__group">
        <span className="controls__group-label">Focus</span>
        <div className="controls__row">
          ...
          {isRunning && !isBreak ? (
            ...
            <button className="ctrl-btn ctrl-btn--primary" onClick={isBreak ? onStop : onStart} title={isBreak ? 'Back to Focus' : (status === 'paused' ? 'Resume' : 'Start')}>
              {isBreak ? (
                /* Tomato icon to go back to focus */
                <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
      ...
      {/* Break group */}
      <div className="controls__group">
        <span className="controls__group-label">Break</span>
        <div className="controls__row">
          <button
            className={`ctrl-btn ctrl-btn--break${type === 'shortBreak' ? ' ctrl-btn--break-active' : ''}`}
          ...
          <button
            className={`ctrl-btn ctrl-btn--break${type === 'longBreak' ? ' ctrl-btn--break-active' : ''}`}
