...
function formatQuestName(q) {
  return (q.nameTemplate ?? '')
    ...
    .replace('{time}', q.targetValue != null ? `${String(q.targetValue).padStart(2, '00')}:00` : '');
}
...
export function QuestsScreen({ questSlate, badgeUnlocks = [], allSessions = [] }) {
  ...
  return (
    <div className="quests-layout">
      ...
      <div className="card quests-daily-card">
        <div className="quests-header">
          <span className="sec-title quests-header__title">Daily Quests</span>
          <span className="quests-header__countdown">
          ...
          return (
            <div key={q.slug} className={`q-item${isComplete ? ' done' : ''}${!isComplete && !isExpired ? ' active' : ''}`}>
              <div className="q-item__top">
                <span className={`q-item__name${isComplete ? ' q-item__name--complete' : ''}`}>
                  {name}
                </span>
                <span className={`q-item__xp${isComplete ? ' q-item__xp--complete' : ''}`}>+{xp} XP</span>
              </div>
      ...
      <div className="quests-right-col">
        {/* Weekly streak */}
        ...
        <div className="card quests-badges">
          <div className="sec-title quests-streak__title">Recent Badges</div>
          {recentBadges.length === 0 ? (
            <p className="quests-badge-placeholder">No badges yet. Keep going!</p>
          ) : (
            recentBadges.map(u => (
        ...
        {/* Countdown */}
        {questSlate && (
          <div className="quests-countdown">
            Resets in <span className="num quests-countdown__num">{formatCountdown(now)}</span>
          </div>
        )}
      </div>
    </div>
